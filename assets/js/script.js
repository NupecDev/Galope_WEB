(function($) {
	...
	if ($('.banner-carousel').length) {
        ...
        // Resume carousel when modal is hidden
  ...
  if ($('.bxslider').length) {
	...
	if($('#video-popup').length){
		...
		$('.video-toggler').on('click', function() {
			$('#video-popup').addClass('popup-visible');
		});
		$(document).keydown(function(e){
	        if(e.keyCode === 27) {
	            $('#video-popup').removeClass('popup-visible');
	            // Destroy video on ESC
	            $('#video-popup iframe').attr('src', '');
	        }
	    });
		//Hide Popup
		$('.close-video,.video-popup .overlay-layer').on('click', function() {
			$('#video-popup').removeClass('popup-visible');
			// Destroy video on close
			$('#video-popup iframe').attr('src', '');
		});
	}
